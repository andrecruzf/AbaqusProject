...
$R_\mathrm{out}$ perturbation. Figure~\ref{fig:ms_point_matrix} illustrates the
point matrix and these Nakazima coordinates.
...
\begin{figure}[H]
    ...
    width $W_X$ across the band and a length $W_Y$ along it. The numbers of DIC
    points $N_X$ and $N_Y$ follow from the measured grid spacings $d_X$ and
    $d_Y$. (b) For the Nakazima test, each deformed point is described by its
    arc-distance coordinate $D$ along a matrix column and its radial distance
    $R_\mathrm{out}$ to the reconstructed punch centre $O'$. The measured
