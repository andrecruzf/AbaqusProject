...
The main result of this semester thesis is the automated numerical workflow
specified in Task~III of the project outline. For each selected configuration,
the pipeline creates the Abaqus model, submits the solver job on Euler, runs
ODB-based post-processing, and synchronises structured result artefacts. The
same interface is used for Nakazima, Marciniak, and Punch-in-Punch simulations,
so that specimen width, sheet thickness, punch radius, punch travel, material
orientation, mass scaling, and mesh refinement can be varied without changing
the post-processing schema.
...
% \begin{figure}[ht]
%   \centering
%   \includegraphics[width=\textwidth]{../study_ms_mr_W200_t1p75_ang0_ps5p0/study_results.pdf}
